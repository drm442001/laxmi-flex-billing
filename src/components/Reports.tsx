  ...
  return (
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 no-print">
        ...
        <button onClick={handlePrint} className="px-4 py-2 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700">🖨️ Print Report</button>
      </div>
      ...
      {loading ? <div className="text-center py-12 text-4xl animate-pulse">📊</div> : !data ? <div className="text-center py-12 text-gray-400">No report data</div> : (
        <>
          {/* Summary Cards */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <SC icon="📄" label="Invoices" value={data.sales.totalInvoices.toString()} sub={formatCurrency(data.sales.totalSales)} color="from-blue-500 to-blue-600"/>
            <SC icon="✅" label="Received" value={formatCurrency(data.payments.totalAmount)} sub={`${data.payments.totalPayments} payments`} color="from-green-500 to-green-600"/>
            <SC icon="⏳" label="Pending" value={formatCurrency(data.sales.totalPending)} color="from-orange-500 to-orange-600"/>
            <SC icon="💸" label="Expenses" value={formatCurrency(data.expenses.totalAmount)} sub={`${data.expenses.totalExpenses} entries`} color="from-red-500 to-red-600"/>
            <SC icon="📈" label="Profit" value={formatCurrency(data.profit)} color={data.profit >= 0 ? "from-emerald-500 to-emerald-600" : "from-red-500 to-red-600"}/>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">📄 Sales Report</h3>
            ...
            {/* Payment Breakdown */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">💳 Payment Breakdown</h3>
              <div className="space-y-2 text-sm">
                <Row label="💵 Cash" value={formatCurrency(data.payments.cashAmount)}/>
                <Row label="📱 UPI" value={formatCurrency(data.payments.upiAmount)}/>
                <Row label="🏦 Bank" value={formatCurrency(data.payments.bankAmount)}/>
                <Row label="📝 Cheque" value={formatCurrency(data.payments.chequeAmount)}/>
                <div className="border-t border-gray-100 pt-2 font-bold"><Row label="Total" value={formatCurrency(data.payments.totalAmount)}/></div>
              </div>
            </div>
            {/* Purchase Report */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">🛒 Purchase Report</h3>
              <div className="space-y-2 text-sm">
            ...
            {/* Expense Report */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">💸 Expense Report</h3>
              <div className="space-y-2 text-sm">
            ...
            {/* Profit Summary */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">📈 Profit Summary</h3>
              <div className="space-y-2 text-sm">
            ...
            {/* Top Customers */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">👑 Top Customers</h3>
              {data.topCustomers.length === 0 ? <p className="text-gray-400 text-sm">No data</p> : (
                <div className="space-y-2">{data.topCustomers.map((c,i)=>(
                  <div key={c.id} className="flex justify-between items-center text-sm">
                    <span className="text-gray-700">{i===0?"🥇":i===1?"🥈":i===2?"🥉":"•"} {c.name}</span>
            ...
            {/* Top Selling Items */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">🏆 Top Selling Items</h3>
              {data.topItems.length === 0 ? <p className="text-gray-400 text-sm">No item data</p> : (
                <div className="space-y-2">{data.topItems.map((item,i)=>(
                  <div key={i} className="flex justify-between items-center text-sm">
                    <span className="text-gray-700">{i<3?"🔥":"•"} {item.description}</span>
          ...
          {data.pendingPayments && data.pendingPayments.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="font-bold text-gray-800 mb-3">⏳ Pending Payments ({data.pendingPayments.length})</h3>
...
function SC({icon,label,value,sub,color}:{icon:string;label:string;value:string;sub?:string;color:string}){
  return <div className={`bg-gradient-to-br ${color} rounded-2xl p-4 text-white`}><div className="text-xl mb-1">{icon}</div><div className="text-lg font-bold truncate">{value}</div><div className="text-[11px] text-white/80">{label}</div>{sub&&<div className="text-[10px] text-white/60">{sub}</div>}</div>;
}
function Row({label,value,cls}:{label:string;value:string;cls?:string}){
  return <div className="flex justify-between"><span className="text-gray-500">{label}</span><span className={cls||"font-medium text-gray-800"}>{value}</span></div>;
}